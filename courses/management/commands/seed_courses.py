import re
from decimal import Decimal
from pathlib import Path
from django.core.management.base import BaseCommand
from django.utils.text import slugify
from courses.models import Course, Unit, Lesson, Progress

DATA_DIR = Path(__file__).resolve().parent.parent.parent / "data"
LESSON_DIR = DATA_DIR / "lessons"
DASH = "\u2014"  # em dash used as title/summary separator


def split_dash(line):
    """Split 'Title — Summary' on the first em dash. Returns (title, summary)."""
    if DASH in line:
        title, summary = line.split(DASH, 1)
        return title.strip(), summary.strip()
    return line.strip(), ""


class Command(BaseCommand):
    help = "Seed the four courses, their units and lessons from data/*.txt, " \
           "and load any full lesson markdown found in data/lessons/."

    def add_arguments(self, parser):
        parser.add_argument("--reset", action="store_true",
                            help="Delete existing courses before seeding.")

    def handle(self, *args, **opts):
        if opts["reset"]:
            Course.objects.all().delete()
            self.stdout.write(self.style.WARNING("Existing courses cleared."))

        # Course order comes from the FILENAME (c11.txt -> order 11), never from
        # alphabetical position: alphabetically c11 sorts before c2, which used
        # to shift every course's number whenever a file was added and attach
        # lesson content (c{order}_l{n}.md) to the wrong course.
        files = sorted(
            (p for p in DATA_DIR.glob("c*.txt")
             if re.fullmatch(r"c(\d+)", p.stem)),
            key=lambda p: int(p.stem[1:]),
        )

        # Self-healing pass: park every existing course on a temporary unique
        # order and slug. Both fields are UNIQUE, so if the database was seeded
        # under a different numbering (the alphabetical-sort era), reassigning
        # in place would trip the constraint mid-loop ("duplicate key ...
        # the-free-market-tradition"). After parking, each course is matched
        # back to its file by title, so lessons and reading Progress survive.
        slug_to_pk = {}
        for c in Course.objects.all():
            slug_to_pk[c.slug] = c.pk
            Course.objects.filter(pk=c.pk).update(
                order=c.pk + 100000, slug=f"__tmp_{c.pk}")

        for path in files:
            self._seed_file(path, int(path.stem[1:]), slug_to_pk)

        # Anything still parked matched no seed file: it is a stale course.
        stale = Course.objects.filter(slug__startswith="__tmp_")
        if stale.exists():
            names = ", ".join(c.title for c in stale)
            self.stdout.write(self.style.WARNING(
                f"  Removed stale course(s): {names}"))
            stale.delete()

        self._load_lessons()
        self.stdout.write(self.style.SUCCESS(
            f"Done. {Course.objects.count()} courses, "
            f"{Unit.objects.count()} units, {Lesson.objects.count()} lessons."))

    def _seed_file(self, path, order, slug_to_pk=None):
        lines = path.read_text(encoding="utf-8").splitlines()
        meta = {}
        body_start = 0
        for i, line in enumerate(lines):
            if line.startswith("@UNIT"):
                body_start = i
                break
            if "|" in line:
                k, v = line.split("|", 1)
                meta[k.strip()] = v.strip()

        slug = slugify(meta["TITLE"])[:50]
        fields = {
            "order": order,
            "slug": slug,
            "title": meta["TITLE"],
            "subtitle": meta.get("SUBTITLE", ""),
            "description": meta.get("DESC", ""),
            "accent": meta.get("ACCENT", "#3a3a3a"),
            "accent_soft": meta.get("ACCENTSOFT", "#ece8e1"),
        }
        # Match an existing (parked) course by the slug it had before this run,
        # so its lessons — and their Progress — are updated in place.
        pk = (slug_to_pk or {}).get(slug)
        if pk is not None and Course.objects.filter(pk=pk).exists():
            Course.objects.filter(pk=pk).update(**fields)
            course = Course.objects.get(pk=pk)
        else:
            course = Course.objects.create(**fields)
        # Idempotent rebuild: update structure in place rather than deleting it,
        # so that reading Progress (a OneToOne on Lesson) survives every re-run.
        # This lets the deploy re-run seed_courses on each push to load new
        # lessons WITHOUT wiping which lessons have been marked as read.
        #
        # Before reassigning slugs, neutralize every existing slug in this course
        # to a unique temporary value. Without this, reordering a course (moving a
        # reused title to an earlier position while its old slug still sits on a
        # later, not-yet-updated row) would trip the (course, slug) UNIQUE
        # constraint mid-loop.
        for pk in list(course.all_lessons.values_list("pk", flat=True)):
            Lesson.objects.filter(pk=pk).update(slug=f"__tmp_{pk}")

        unit = None
        lesson_counter = 0
        sub_counter = 0
        order_in_unit = 0
        used_slugs = set()
        seen_unit_numbers = []
        seen_lesson_numbers = []
        for line in lines[body_start:]:
            line = line.rstrip()
            if not line:
                continue
            if line.startswith("@UNIT"):
                _, num, title, subtitle = (line.split("|") + ["", "", ""])[:4]
                unit, _ = Unit.objects.update_or_create(
                    course=course, number=int(num),
                    defaults={
                        "title": title.strip(),
                        "subtitle": subtitle.strip(),
                        "order": int(num),
                    },
                )
                seen_unit_numbers.append(int(num))
                order_in_unit = 0
                continue
            # A "@SUB|Title — Summary" line is a sub-lesson of the preceding main
            # lesson: it takes a decimal number (80 -> 80.1, 80.2, ...) so extra
            # depth can be inserted without renumbering anything that follows.
            is_sub = line.startswith("@SUB|")
            if is_sub:
                if lesson_counter == 0:
                    continue  # a sub-lesson with no parent yet: skip defensively
                sub_counter += 1
                number = Decimal(lesson_counter) + (Decimal(sub_counter) / Decimal(10))
                line = line.split("|", 1)[1]
            else:
                lesson_counter += 1
                sub_counter = 0
                number = Decimal(lesson_counter)
            order_in_unit += 1
            title, summary = split_dash(line)
            base = slugify(title)[:60] or f"lesson-{lesson_counter}"
            slug = base
            n = 2
            while slug in used_slugs:
                slug = f"{base}-{n}"
                n += 1
            used_slugs.add(slug)
            lesson, _ = Lesson.objects.update_or_create(
                course=course, course_lesson_number=number,
                defaults={
                    "unit": unit,
                    "order_in_unit": order_in_unit,
                    "title": title,
                    "summary": summary,
                    "slug": slug,
                },
            )
            seen_lesson_numbers.append(number)
            Progress.objects.get_or_create(lesson=lesson)

        # Remove any structure no longer present in the seed file (rare). Lessons
        # that still exist keep their row — and therefore their Progress.
        course.all_lessons.exclude(
            course_lesson_number__in=seen_lesson_numbers).delete()
        course.units.exclude(number__in=seen_unit_numbers).delete()

        self.stdout.write(f"  Course {order}: {course.title} "
                          f"({course.units.count()} units, {lesson_counter} lessons)")

    def _load_lessons(self):
        """Load full lesson content from data/lessons/cN_LL.md if present.
        Filename convention: c{course_order}_l{course_lesson_number}.md
        Content is treated as purely a function of the lesson files: any lesson
        without a backing file has its content cleared, so restructuring a
        course never leaves stale text attached to a reused lesson slot."""
        loaded = 0
        loaded_ids = set()
        if LESSON_DIR.exists():
            for f in sorted(LESSON_DIR.glob("c*_l*.md")):
                m = re.match(r"c(\d+)_l(\d+(?:\.\d+)?)", f.stem)
                if not m:
                    continue
                c_order, l_num = int(m.group(1)), Decimal(m.group(2))
                try:
                    lesson = Lesson.objects.get(
                        course__order=c_order, course_lesson_number=l_num)
                except Lesson.DoesNotExist:
                    continue
                lesson.content = f.read_text(encoding="utf-8")
                lesson.save(update_fields=["content"])
                loaded_ids.add(lesson.pk)
                loaded += 1
        # Clear stale content from any lesson that no longer has a backing file.
        cleared = (Lesson.objects.exclude(pk__in=loaded_ids)
                   .exclude(content="").update(content=""))
        if loaded:
            self.stdout.write(self.style.SUCCESS(f"  Loaded {loaded} full lesson(s)."))
        if cleared:
            self.stdout.write(self.style.WARNING(f"  Cleared {cleared} stale lesson(s)."))
