...
class RecallNote(models.Model):
    """Free-recall note for a lesson: the reader writes what they remember
    after finishing, below the footnotes. One note per lesson."""
    lesson = models.OneToOneField(Lesson, related_name="recall_note", on_delete=models.CASCADE)
    text = models.TextField(blank=True, default="")
    updated_at = models.DateTimeField(auto_now=True)

    def __str__(self):
        return f"Recall for {self.lesson.title} ({len(self.text)} chars)"
